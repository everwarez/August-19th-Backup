...
local function UpdateHandler(frame, elapsed)
	...
	if self.ActiveTab=="Inventory" and Appraiser.ActiveSellingItem and buttonPost:IsEnabled() then
		...
		elseif stack_count*stack_size>GetItemCount(Appraiser.ActiveSellingItem.itemid) then
			buttonPost.soft_disabled=true
			buttonPost.tooltip = ("|cffffaa00Warning:|r Unable to post |cffffffff%d|r stack%s of |cffffffff%d|r %s.|nThat's |cffffffff%d|r total, and you only have |cffffffff%d|r."):format(stack_count,stack_count>1 and "s" or "",stack_size,link,stack_count*stack_size,GetItemCount(Appraiser.ActiveSellingItem.itemid))
		else
			buttonPost.soft_disabled=false
			if stack_count*stack_size==1 then
				buttonPost.tooltip = ("Post |cffffffff1|cffffcc00 auction of |cffffee66%s|r."):format(link)
			elseif stack_size==1 or stack_count==1 then
				buttonPost.tooltip = ("Post |cffffffff%d|r stack%s of |cffffffff%d %s."):format(stack_count,stack_count>1 and "s" or "",stack_size,link)
			else
				buttonPost.tooltip = ("Post |cffffffff%d|r stacks of |cffffffff%d %s, a total of |cffffffff%d|cffffdd00."):format(stack_count,stack_size,link,stack_count*stack_size)
			end
		end
	else
		buttonPost.tooltip = nil
