...
function AutoEquip:CanUseUniqueItem(test_itemid, current_itemid)
	...
	if not uniqueness_fam then 
		...
		return true,true
	elseif uniqueness_fam<0 and current_itemid == test_itemid then 
		-- item with family -1, unique to itself
		return false,true
	--elseif current_itemid and maxEquip==1 and uniqueness_fam>0 and uniqueness_fam == AutoEquip:GetItemUniqueness(current_itemid) then
		-- same unique family, only one item allowed
	--	return false,true
	--elseif maxEquip>1  then
	elseif uniqueness_fam then
		-- more than one allowed, count all and see
		return (AutoEquip.UniqueEquipped[uniqueness_fam] or 0)<maxEquip,false
	else
		return true,true
	end
end
